...
def welcome_msg():
    line_console()
    print("welcome to".upper().center(150, '-'))
    line_console()
    print("T20 Cricket league simulator".upper().center(150, '-'))
    line_console()
    line_console()

def check():
    chk_input = input("\nType Enter to continue: ")
    if chk_input.lower() == "enter":
        return True
...
def instruction():
    ...
    if check():
        ...
        print("The game has 2 stages\n\ti.the auction\n\tii.the league".upper())
        print("\nsimilar to the structure of the ipl"
              "\n1.first pick your team by typing the abbreviation of ipl teams"
              "\n2.the auction:the auction begins by displaying the number of players and then proceed to display the players."
              "\nAfter the current player to be bid on is displayed you can type in the maximum bid for that player."
              "\nYour starting budget 1540 coins.Each auction round consists of 8 players being bid on and after each round you can choose to view yor existing team and budget when prompted."
              "\n3.the league:before every match you have the option of rearranging your team's batting and bowling order."
              "\nYou can also view the points table and the players' stats table or your team's detailed stats.".title())
        print("\n\nWhen you decide to play a match you have 2 options\n\t1.detailed scorecard"
              "\n\t2.summary".title())
...
def showtable(tableteams):
    print("pos".ljust(7), "team".ljust(7), "played".ljust(7), "won".ljust(7), "lost".ljust(7), "pts".ljust(7),
          "nrr".ljust(7))
    for i in range(8):
        print(str(i + 1).ljust(7), tableteams[i].name.ljust(7), str(tableteams[i].played).ljust(7),
              str(tableteams[i].won).ljust(7), str(tableteams[i].lost).ljust(7), str(tableteams[i].pts).ljust(7),
              str(tableteams[i].nrr).ljust(7))


def updatetable(tableteams):
    for i in range(8):
        for j in range(7 - i):
            if ((tableteams[j].pts < tableteams[j + 1].pts) or (
                    tableteams[j].pts == tableteams[j + 1].pts and tableteams[j].nrr < tableteams[j + 1].nrr)):
...
def playerstats(teams, mvp, batstats, bowlstats):
    ...
    for i in range(len(batstats)):
        for j in range(len(batstats) - 1 - i):
            ...
            if bowlstats[j].totalwkts < bowlstats[j + 1].totalwkts or (
                    bowlstats[j].totalwkts == bowlstats[j + 1].totalwkts and bowlstats[j].totalbowlruns > bowlstats[
                j + 1].totalbowlruns):
    ...
    print("pos", "mvp".ljust(20), "pts".ljust(5), "pos", "batsman".ljust(20), "runs".ljust(5), "pos",
          "bowler".ljust(20), "wkts")
    for l in range(10):
        print(str(l + 1).ljust(3), mvp[l].name.ljust(20), str(mvp[l].pts).ljust(5), str(l + 1).ljust(3),
              batstats[l].name.ljust(20), str(batstats[l].totalruns).ljust(5), str(l + 1).ljust(3),
              bowlstats[l].name.ljust(20), str(bowlstats[l].totalwkts).ljust(5))


def teamstats(team):
    for p in team.players:
        p.calcpts()
        print(p.name.ljust(20), ":", str(p.pts).ljust(5),
              (str(p.totalruns) + '(' + str(p.totalballsfaced) + ')').ljust(9), str(p.avg).ljust(8),
              (str(p.totalwkts) + '-' + str(p.totalbowlruns)).ljust(8), int(p.totalballs / 6), '.', p.totalballs % 6)
...
def season():
    ...
    for i in range(5):
        if check():
            ...
            print("\nPlease type enter properly\n")
    username = input('CHOOSE YOUR TEAM: ')
    for i in range(9999):
        if i % 3 == 0:
        ...
        with open('seasons.csv', 'a') as seasonfile:
            line = '\n{0[0].name},{0[1].name},{0[2].name},{0[3].name},{0[4].name},{0[5].name},{0[6].name},{0[7].name},{1[0].name},{2[0].name},{3[0].name}'.format(
                tableteams, mvp, batstats, bowlstats)
