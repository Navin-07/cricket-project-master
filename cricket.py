...
class player:
    ...
    def calcpts(self):
        ...
        self.pts = int((2 * self.totalruns) - (self.totalballsfaced) - (10 * self.outs) + (30 * self.totalwkts) + (
                    (self.totalballs * 1.67) - self.totalbowlruns))
        k = 0
...
def scorecard(batsmen, bowlers, teamscore):
    ...
    for i in range(11):
        if bowlers[i].bowlballs > 0:
            print(bowlers[i].name.ljust(20),
                  str(int((bowlers[i].bowlballs) / 6)) + '.' + str((bowlers[i].bowlballs) % 6), " ".ljust(5),
                  str(bowlers[i].bowlwkts).ljust(6), str(bowlers[i].bowlruns).ljust(6))
        else:
            pass
...
def summary(inn1, inn2, team1, team2):
    ...
    for i in range(2):
        for j in range(1 + i, 11):
            ...
            if b1[11 - j].bowlwkts > b1[10 - j].bowlwkts or (
                    b1[11 - j].bowlwkts == b1[10 - j].bowlwkts and b1[11 - j].bowlruns < b1[10 - j].bowlruns):
                b1[11 - j], b1[10 - j] = Switch(b1[11 - j], b1[10 - j])
            else:
                pass

            if (b2[11 - j].bowlwkts > b2[10 - j].bowlwkts) or (
                    b2[11 - j].bowlwkts == b2[10 - j].bowlwkts and b2[11 - j].bowlruns < b2[10 - j].bowlruns):
                b2[11 - j], b2[10 - j] = Switch(b2[11 - j], b2[10 - j])
            else:
                pass

    print("\t", team1.name, "innings:", inn1, '\n', p1[0].name.ljust(20), ":",
          str(p1[0].batscore) + '(' + str(p1[0].ballsfaced) + ')', "\t", b2[0].name.ljust(20), ":", b2[0].bowlwkts,
          "for", b2[0].bowlruns, "\n", p1[1].name.ljust(20), ":",
          str(p1[1].batscore) + '(' + str(p1[1].ballsfaced) + ')', "\t", b2[1].name.ljust(20), ":", b2[1].bowlwkts,
          "for", b2[1].bowlruns, "\n")
    print("\t", team2.name, "innings:", inn2, '\n', p2[0].name.ljust(20), ":",
          str(p2[0].batscore) + '(' + str(p2[0].ballsfaced) + ')', "\t", b1[0].name.ljust(20), ":", b1[0].bowlwkts,
          "for", b1[0].bowlruns, "\n", p2[1].name.ljust(20), ":",
          str(p2[1].batscore) + '(' + str(p2[1].ballsfaced) + ')', "\t", b1[1].name.ljust(20), ":", b1[1].bowlwkts,
          "for", b1[1].bowlruns, "\n")
